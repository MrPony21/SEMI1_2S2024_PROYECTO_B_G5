...
const Home = () => {
    ...
    useEffect( () => {
        ...
        const fetchTravels = async () =>{
            try {
                const res = await fetch(`${API_BACKEND}/travel/all`, {
                    method: 'GET',
                    headers: {
    ...
    return (
        <div className="principal-container">
            ...
            <div className="grid-container">
                {travels.map((element) => (
                    <div key={element.travel_id} className="grid-item1">
                        <Card_travel id={element.travel_id} title={element.travel_name} description={element.travel_description} src={element.travel_image_link} price={element.travel_cost} rate={element.travel_review} /> 
                    </div>
                ))}
            </div>
        </div>
